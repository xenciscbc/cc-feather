...
## 0.4.0 independent handoff and delegation setup

Setup now inspects status before asking for missing intent, and manages handoff maintenance policy and delegation policy plus agents independently. The CLI accepts --component handoff|delegation|both; schema v3 stores separate ownership records. Legacy v1/v2 combined guidance is split without losing the handoff reminder or saved role/review choices. Handoff-only operations preserve unrelated agent conflicts and delegation changes. Combined operations share one preview/apply transaction.

The executor changed only scripts/feather_config.py and tests/test_feather_config.py. Main reviewed the actual diff, identified and closed legacy split/ownership/CRLF and mixed-state gaps, and integrated separate templates, skills and documentation. Windows configuration tests: 30 total, 28 passed, 2 platform skips. Ubuntu/WSL configuration tests: all 30 passed. Coverage includes component isolation, mixed installation states, legacy role names, LF/CRLF split, rollback and POSIX permissions.

Main also ran a real CLI subprocess lifecycle in temporary Unicode/space paths using the historical v2 policy from Git HEAD converted to CRLF: handoff-only removal retained every role byte, model change worked, handoff reinstall worked, delegation removal retained handoff, and final handoff removal restored the original user guidance. Plugin manifest validation and local Markdown-link checks passed; git diff --check was clean. No handoff runtime code or data format changed, and no live Claude installation or interactive setup/model dispatch was performed.

## Component setup review fixes

Review found and closed three issues: agent inspection errors now remain delegation-specific so handoff status is still available; unreadable/unsafe settings diagnostics report unknown overrides without hiding component status; installing both with an already installed delegation rejects a conflicting review-mode argument instead of reporting an unsaved mode. Session export now distinguishes policy-schema migration from role-name migration: valid v2 native role definitions export read-only without a setup update, while legacy feather-* names still require migration.

Validation after all fixes: Windows configuration suite ran 33 cases, with 31 passed and 2 platform skips. Five targeted Ubuntu/WSL regression cases passed, covering the review fixes and v1/v2 migration behavior. V2 LF/CRLF export tests verify unchanged installation bytes. Main additionally checked 16 install/remove order and original-newline roundtrips; all restored the original guidance bytes. git diff --check passed. Live Claude setup interaction and model dispatch remain untested.
